...
exports.validateSerial = exports.sendTestNotification = exports.getNotificationConfig = exports.saveNotificationConfig = exports.setGeneralConfig = exports.adminGetStats = exports.getLeaderboard = exports.reviewSale = exports.getMyTeam = exports.registerSellerSale = exports.submitSellerQuiz = exports.pickWinner = exports.importCodes = exports.registerClient = void 0;
...
exports.registerClient = functions.https.onCall(async (data, context) => {
    // 1. Sanitize Input
...
exports.importCodes = functions.https.onCall(async (data, context) => {
    ...
    for (const item of codes) {
        ...
        count++;
        if (count >= 490)
            break; // Safety break for single batch. For massive uploads, client should chunk.
    }
    await batch.commit();
...
// --- SELLER: SUBMIT QUIZ ---
exports.submitSellerQuiz = functions.https.onCall(async (data, context) => {
    assertAuth(context);
...
exports.registerSellerSale = functions.https.onCall(async (data, context) => {
    assertAuth(context);
...
// --- SELLER: GET MY TEAM ---
exports.getMyTeam = functions.https.onCall(async (data, context) => {
    var _a;
    assertAuth(context);
    const uid = context.auth.uid;
    ...
    if (!sellerDoc.exists)
        return { team: [] };
    const myCi = (_a = sellerDoc.data()) === null || _a === void 0 ? void 0 : _a.ci;
    if (!myCi)
        return { team: [] };
...
// --- ADMIN: REVIEW SALE ---
exports.reviewSale = functions.https.onCall(async (data, context) => {
    assertAdmin(context);
    ...
    if (action === 'REJECT' && (saleData === null || saleData === void 0 ? void 0 : saleData.status) !== 'REJECTED') {
        await db.runTransaction(async (t) => {
            t.update(saleRef, { status: 'REJECTED' });
            const sellerRef = db.collection('sellers').doc(saleData === null || saleData === void 0 ? void 0 : saleData.sellerId);
            t.update(sellerRef, { totalSales: admin.firestore.FieldValue.increment(-1) });
        });
    }
    else if (action === 'APPROVE') {
        if ((saleData === null || saleData === void 0 ? void 0 : saleData.status) === 'REJECTED') {
            await db.runTransaction(async (t) => {
...
// --- ADMIN: CONFIGURATION ---
exports.setGeneralConfig = functions.https.onCall(async (data, context) => {
    assertAdmin(context);
...
exports.saveNotificationConfig = functions.https.onCall(async (data, context) => {
    var _a;
    assertAdmin(context);
    // Validate basic structure
    const { whatsapp, email } = data;
    if (!whatsapp || !email)
        throw new functions.https.HttpsError('invalid-argument', 'Faltan configuraciones.');
    ...
    await db.collection('settings').doc('notifications').set({
        ...
        email,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedBy: (_a = context.auth) === null || _a === void 0 ? void 0 : _a.uid
    });
    return { success: true };
});
// NEW: Get Notification Config (for UI population)
exports.getNotificationConfig = functions.https.onCall(async (data, context) => {
    assertAdmin(context);
...
exports.sendTestNotification = functions.https.onCall(async (data, context) => {
    ...
    if (!config)
        throw new functions.https.HttpsError('failed-precondition', 'No hay configuración guardada.');
    if (type === 'whatsapp') {
        const { token, phoneId } = config.whatsapp || {};
        if (!token || !phoneId)
            throw new functions.https.HttpsError('failed-precondition', 'Faltan datos de WhatsApp.');
        try {
            return { success: true, message: 'Configuración válida. (Simulación de envío realizada)' };
        }
        catch (e) {
            console.error("WhatsApp Error", ((_a = e.response) === null || _a === void 0 ? void 0 : _a.data) || e.message);
            throw new functions.https.HttpsError('unknown', 'Error conectando con Meta API.');
        }
    }
    if (type === 'email') {
        if (!((_b = config.email) === null || _b === void 0 ? void 0 : _b.host))
            throw new functions.https.HttpsError('failed-precondition', 'Faltan datos SMTP.');
...
exports.validateSerial = functions.https.onCall(async (data, context) => {
    ...
    if (!serial)
        throw new functions.https.HttpsError('invalid-argument', 'Serial vacío.');
    try {
        // 1. Check Valid Codes
        ...
        let coupons = 1;
        let description = '';
        if (codeData === null || codeData === void 0 ? void 0 : codeData.model) {
        ...
        return {
            status: 'AVAILABLE',
            model: codeData === null || codeData === void 0 ? void 0 : codeData.model,
            coupons,
            description
        };
    }
